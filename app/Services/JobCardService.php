...
class JobCardService
{
    ...
    public function create(Vehicle $vehicle, User $creator, array $payload): JobCard
    {
        ...
        $jobCard = DB::transaction(function () use ($vehicle, $creator, $payload): JobCard {
            ...
            $selectedStages
                ->each(function (array $selectedStage, int $index) use ($jobCard, $stageTemplates, $receivedAt): void {
                    ...
                    $technicianIds = collect($selectedStage['technician_ids'] ?? [])
                        ->filter()
                        ->map(fn ($id) => (int) $id)
                        ->unique()
                        ->values()
        ...
        return $jobCard->load(['vehicle.client', 'currentJobStage.stage', 'jobStages.stage', 'jobStages.assignedTechnician', 'jobStages.technicians']);
    }
    ...
    public function updateFutureStage(JobStage $jobStage, array $payload, User $actor): JobStage
    {
        ...
        $technicianIds = collect(Arr::get($payload, 'technician_ids', []))
            ->filter()
            ->map(fn ($id) => (int) $id)
            ->unique()
            ->values()
        ...
        $this->jobCardAuditService->record(
            ...
            $jobStage,
            [
                'assigned_technician_id' => $jobStage->assigned_technician_id,
        ...
        return $jobStage->fresh(['stage', 'assignedTechnician', 'technicians']);
    }
